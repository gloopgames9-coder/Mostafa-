...
module.exports = {
  config: {
    ...
    version: "4.9",
    author: "Megumi",
    shortDescription: "Show all available user commands",
    longDescription: "Displays a clean categorized list of commands available for all members.",
    category: "system",
  ...
  onStart: async function ({ message, args, prefix }) {
    ...
    // تجميع الأوامر المتاحة للأعضاء فقط (role: 0 أو غير محدد)
    for (const [name, cmd] of allCommands) {
      const cmdRole = cmd.config.role !== undefined ? cmd.config.role : 0;
      
      // إخفاء أوامر الأدمن وإظهار أوامر الأعضاء فقط
      if (cmdRole === 0) {
    ...
    for (const cat of sortedCategories) {
      if (categories[cat].length === 0) continue; // تخطي القوائم الفارغة
